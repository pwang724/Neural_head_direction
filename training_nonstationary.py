...
class RNN:
    ...
    def __init__(self, sess, opts):
        ...
        self.y = tf.placeholder(tf.float32, [None, time_steps, state_size+velocity_size], name='output_placeholder')

        inputs_series = tf.unstack(self.x, axis=1)
        labels_series = tf.unstack(self.y, axis=1)

        support_size = rnn_size - state_size
        self.W_i_aa = tf.constant(np.eye(state_size), dtype= tf.float32)
        self.W_i_ab = tf.get_variable('W_ab', shape= [state_size, support_size])
        self.W_i_ba = tf.get_variable('W_ba', shape= [velocity_size, state_size])
        self.W_i_bb = tf.get_variable('W_bb', shape= [support_size, support_size])
    ...
    def rnn(self, h_prev, input, name, state_size):
        aa = tf.matmul(input[:,:state_size], self.W_i_aa)
        ba = tf.matmul(input[:,state_size:], self.W_i_ba)
        ab = tf.matmul(input[:,:state_size], self.W_i_ab)
        bb = tf.matmul(input[:,state_size:], self.W_i_bb)
        vec_in = tf.concat([aa + ba, ab + bb], axis= 1)
